...
#ifdef TOOLS_ENABLED
// Returns the features that a project must have when opened with this build of
// Godot. This is used by the project manager to provide the initial_settings
// for config/features.
...
// Returns the features supported by this build of Godot. Includes all required
// features.
...
// Returns the features that both this project has and this build of Godot has,
// ensuring required features exist.
const PackedStringArray ProjectSettings::_trim_to_supported_features(
    const PackedStringArray &p_project_features) {
...
String ProjectSettings::localize_path(const String &p_path) const {
  ...
  if (resource_path.is_empty() ||
      (path.is_absolute_path() && !path.begins_with(resource_path))) {
    return path;
  }
  ...
  if (dir->change_dir(path) == OK) {
    ...
    // This is important to ensure that we do not wrongly localize the resource
    // path in an absolute path that just happens to contain this string but
    // points to a different folder (e.g. "/my/project" as resource_path would
    // be contained in
    ...
    // DirAccess::get_current_dir() is not guaranteed to return a path that with
    // a trailing '/', so we must make sure we have it as well in order to
    // compare with 'res_path'.
    cwd = cwd.path_join("");
...
void ProjectSettings::set_initial_value(const String &p_name,
                                        const Variant &p_value) {
  ERR_FAIL_COND_MSG(
      !props.has(p_name),
      vformat("Request for nonexistent project setting: '%s'.", p_name));

  // Duplicate so that if value is array or dictionary, changing the setting
  // will not change the stored initial value.
  props[p_name].initial = p_value.duplicate();
}

void ProjectSettings::set_restart_if_changed(const String &p_name,
                                             bool p_restart) {
  ERR_FAIL_COND_MSG(
      !props.has(p_name),
      vformat("Request for nonexistent project setting: '%s'.", p_name));
...
void ProjectSettings::set_ignore_value_in_docs(const String &p_name,
                                               bool p_ignore) {
  ERR_FAIL_COND_MSG(
      !props.has(p_name),
      vformat("Request for nonexistent project setting: '%s'.", p_name));
...
void ProjectSettings::add_hidden_prefix(const String &p_prefix) {
  ERR_FAIL_COND_MSG(hidden_prefixes.has(p_prefix),
                    vformat("Hidden prefix '%s' already exists.", p_prefix));
      ...
      if (dot != -1) {
        ...
        for (int i = 1; i < s.size(); i++) {
          ...
          if (!feature_overrides.has(s[0])) {
            feature_overrides[s[0]] =
                LocalVector<Pair<StringName, StringName>>();
          }
...
Variant ProjectSettings::get_setting_with_override_and_custom_features(
    const StringName &p_name, const Vector<String> &p_features) const {
  _THREAD_SAFE_METHOD_

  StringName name = p_name;
  if (feature_overrides.has(name)) {
    const LocalVector<Pair<StringName, StringName>> &overrides =
        feature_overrides[name];
...
Variant
ProjectSettings::get_setting_with_override(const StringName &p_name) const {
  _THREAD_SAFE_METHOD_
  ...
  if (overrides) {
    for (uint32_t i = 0; i < overrides->size(); i++) {
      ...
      // Custom features are checked in OS.has_feature() already. No need to
      // check twice.
      const RBMap<StringName, VariantContainer>::Element *override_prop =
          props.find((*overrides)[i].second);
      if (override_prop) {
...
void ProjectSettings::_get_property_list(List<PropertyInfo> *p_list) const {
  ...
  for (const _VCSort &base : vclist) {
    if (custom_prop_info.has(base.name)) {
      ...
      PropertyInfo info(base.type, base.name, PROPERTY_HINT_NONE, "",
                        base.flags);

      const PropertyInfo *pi = editor_settings_info.getptr(
          base.name.trim_prefix(EDITOR_SETTING_OVERRIDE_PREFIX));
      if (pi) {
        info.usage = pi->usage;
        info.hint = pi->hint;
      ...
      p_list->push_back(PropertyInfo(base.type, base.name, PROPERTY_HINT_NONE,
                                     "", base.flags));
    }
    ...
    if (setting_overrides.has(base.name)) {
      for (const _VCSort &over : setting_overrides.get(base.name)) {
        if (custom_prop_info.has(over.name)) {
          ...
          p_list->push_back(PropertyInfo(over.type, over.name,
                                         PROPERTY_HINT_NONE, "", over.flags));
        }
      }
    }
  }
...
void ProjectSettings::_queue_changed(const StringName &p_name) {
  ...
  if (!MessageQueue::get_singleton() ||
      MessageQueue::get_singleton()->get_max_buffer_usage() == 0) {
    return;
  }
...
bool ProjectSettings::load_resource_pack(const String &p_pack,
                                         bool p_replace_files, int p_offset) {
  return ProjectSettings::_load_resource_pack(p_pack, p_replace_files, p_offset,
                                              false);
}
...
bool ProjectSettings::_load_resource_pack(const String &p_pack,
  ...
  if (p_pack == "res://") {
    // Loading the resource directory as a pack source is reserved for internal
    // use only.
    return false;
  }

  if (!p_main_pack && !using_datapack &&
      !OS::get_singleton()->get_resource_dir().is_empty()) {
    // Add the project's resource file system to PackedData so directory access
    // keeps working when the game is running without a main pack, like in the
    // editor or on Android.
  ...
  bool ok = PackedData::get_singleton()->add_pack(p_pack, p_replace_files,
                                                  p_offset) == OK;
  if (!ok) {
    return false;
  }
  ...
  if (project_loaded) {
    // This pack may have declared new global classes (make sure they are picked
    // up).
...
void ProjectSettings::_convert_to_last_version(int p_from_version) {
  ...
  if (p_from_version <= 3) {
    ...
    // Check if we still have legacy boot splash (removed in 4.6), map it to new
    // project setting, then remove legacy setting.
    if (has_setting("application/boot_splash/fullsize")) {
      set_setting("application/boot_splash/stretch_mode",
                  RenderingServer::map_scaling_option_to_stretch_mode(
                      get_setting("application/boot_splash/fullsize")));
 ...
 *  - On Android, look for 'assets.sparsepck' and try loading it, if it doesn't
 ...
 * we are running from source code. If not found and `p_upwards` is true, look
 * for project files in parent folders up to the system root (used to run a game
 * from command line while in a subfolder). If a project file is found, load it
 * or fail. If nothing was found, error out.
 */
Error ProjectSettings::_setup(const String &p_path, const String &p_main_pack,
                              bool p_upwards, bool p_ignore_override) {
  if (!OS::get_singleton()->get_resource_dir().is_empty()) {
    // OS will call ProjectSettings->get_resource_path which will be empty if
    // not overridden! If the OS would rather use a specific location, then it
    // will not be empty.
    resource_path =
        OS::get_singleton()->get_resource_dir().replace_char('\\', '/');
    if (!resource_path.is_empty() &&
  ...
  if (!p_main_pack.is_empty()) {
    ...
    ERR_FAIL_COND_V_MSG(
        !ok, ERR_CANT_OPEN,
        vformat("Cannot open resource pack '%s'.", p_main_pack));

    Error err = _load_settings_text_or_binary("res://project.godot",
                                              "res://project.binary");
    ...
    if (err == OK && !p_ignore_override) {
      ...
      if (!disable_override) {
        _load_settings_text(
            p_main_pack.get_base_dir().path_join("override.cfg"));
      }
    }
  ...
  if (!exec_path.is_empty()) {
    ...
    // Based on the OS, it can be the exec path + '.pck' (Linux w/o extension,
    // macOS in .app bundle) or the exec path's basename + '.pck' (Windows). We
    // need to test both possibilities as extensions for Linux binaries are
    // optional (so both 'mygame.bin' and 'mygame' should be able to find
    // 'mygame.pck').
    ...
    if (!found) {
      ...
      found = _load_resource_pack(
                  OS::get_singleton()->get_bundle_resource_dir().path_join(
                      exec_basename + ".pck"),
                  false, 0, true) ||
              _load_resource_pack(
                  OS::get_singleton()->get_bundle_resource_dir().path_join(
    ...
    if (!found) {
      ...
      found = _load_resource_pack(exec_dir.path_join(exec_basename + ".pck"),
                                  false, 0, true) ||
    ...
    if (found) {
      ...
      if (err == OK && !p_ignore_override) {
        ...
        if (!disable_override) {
          ...
          _load_settings_text(
              exec_path.get_base_dir().path_join("override.cfg"));
        }
      }
#endif // OVERRIDE_ENABLED
      return err;
  ...
  if (!resource_path.is_empty()) {
    ...
    err = _load_settings_text_or_binary(
        resource_path.path_join("project.godot"),
        resource_path.path_join("project.binary"));
    if (err == OK && !p_ignore_override) {
      // Optional, we don't mind if it fails.
#ifdef OVERRIDE_ENABLED
      bool disable_override =
  ...
  ERR_FAIL_COND_V_MSG(
      d.is_null(), ERR_CANT_CREATE,
      vformat("Cannot create DirAccess for path '%s'.", p_path));
  d->change_dir(p_path);
  ...
  while (true) {
    ...
    resource_path = resource_path.replace_char(
        '\\', '/'); // Windows path to Unix path just in case.
    err =
        _load_settings_text_or_binary(current_dir.path_join("project.godot"),
                                      current_dir.path_join("project.binary"));
    if (err == OK) {
#ifdef OVERRIDE_ENABLED
  ...
  if (err == OK && !p_ignore_override) {
    ...
    if (!disable_override) {
      String custom_settings =
          GLOBAL_GET("application/config/project_settings_override");
...
Error ProjectSettings::_load_settings_binary(const String &p_path) {
  ...
  f->get_buffer(hdr, 4);
  ERR_FAIL_COND_V_MSG(
      (hdr[0] != 'E' || hdr[1] != 'C' || hdr[2] != 'F' || hdr[3] != 'G'),
      ERR_FILE_CORRUPT,
...
Error ProjectSettings::_load_settings_text(const String &p_path) {
  ...
  if (f.is_null()) {
    ...
    // missing This needs to be streamlined if we want decent error reporting
  ...
  while (true) {
    ...
    if (err == ERR_FILE_EOF) {
      ...
      last_save_time = FileAccess::get_modified_time(
          get_resource_path().path_join("project.godot"));
      return OK;
    }
    ERR_FAIL_COND_V_MSG(
        err != OK, err,
        vformat("Error parsing '%s' at line %d: %s File might be corrupted.",
    ...
    if (!assign.is_empty()) {
      if (section.is_empty() && assign == "config_version") {
        ...
        ERR_FAIL_COND_V_MSG(
            config_version > CONFIG_VERSION, ERR_FILE_CANT_OPEN,
            vformat("Can't open project at '%s', its `config_version` (%d) is "
                    "from a more recent and incompatible version of the "
                    "engine. Expected config version: %d.",
...
Error ProjectSettings::_load_settings_text_or_binary(const String &p_text_path,
  ...
  if (err == OK) {
    ...
    ERR_PRINT(
        vformat("Couldn't load file '%s', error code %d.", p_bin_path, err));
  }
  ...
  if (err == OK) {
    ...
    const PackedStringArray features =
        get_setting("application/config/features");
    for (const String &feat : features) {
      if (feat.contains_char('.') && feat.get_slice_count(".") == 2) {
        ...
        // Major version is irrelevant, but the extra check ensures that the
        // feature is in fact a version string.
        if (major_version == 4 && minor_version < 6) {
          // Enable MeshInstance3D compat for projects created before 4.6.
          set_setting("animation/compatibility/"
                      "default_parent_skeleton_in_mesh_instance_3d",
                      true);
        }
        break;
      }
    ...
    ERR_PRINT(
        vformat("Couldn't load file '%s', error code %d.", p_text_path, err));
  }
...
Error ProjectSettings::_save_settings_binary(
    const String &p_file, const RBMap<String, List<String>> &p_props,
    const CustomMap &p_custom, const String &p_custom_features) {
  Error err;
  Ref<FileAccess> file = FileAccess::open(p_file, FileAccess::WRITE, &err);
  ERR_FAIL_COND_V_MSG(err != OK, err,
                      vformat("Couldn't save project.binary at '%s'.", p_file));
  ...
  if (!p_custom_features.is_empty()) {
    // Store how many properties are saved, add one for custom features, which
  ...
  for (const KeyValue<String, List<String>> &E : p_props) {
    for (const String &key : E.value) {
      ...
      ERR_FAIL_COND_V_MSG(err != OK, ERR_INVALID_DATA,
...
Error ProjectSettings::_save_settings_text(
  ...
  ERR_FAIL_COND_V_MSG(err != OK, err,
                      vformat("Couldn't save project.godot - %s.", p_file));
...
Error ProjectSettings::_save_custom_bnd(
    const String &p_file) { // add other params as dictionary and array?
...
Error ProjectSettings::save_custom(const String &p_path,
...
#ifdef TOOLS_ENABLED
  PackedStringArray project_features =
      get_setting("application/config/features");
  ...
  const String rendering_api =
      has_setting("rendering/renderer/rendering_method")
          ? (String)get_setting("rendering/renderer/rendering_method")
          : String();
  ...
  if (_csproj_exists(get_resource_path())) {
    // If there is a csproj file, add the C# feature if it doesn't already
    // exist.
  ...
  for (const KeyValue<String, Variant> &E : p_custom) {
    ...
    RBMap<StringName, VariantContainer>::Iterator global_prop =
        props.find(E.key);
  ...
  if (p_path.ends_with(".godot") || p_path.ends_with("override.cfg")) {
    ...
    ERR_FAIL_V_MSG(ERR_FILE_UNRECOGNIZED,
                   vformat("Unknown config file format: '%s'.", p_path));
  }
}

Variant _GLOBAL_DEF(const String &p_var, const Variant &p_default,
                    bool p_restart_if_changed, bool p_ignore_value_in_docs,
                    bool p_basic, bool p_internal) {
  Variant ret;
  ...
  ProjectSettings::get_singleton()->set_restart_if_changed(
      p_var, p_restart_if_changed);
  ProjectSettings::get_singleton()->set_ignore_value_in_docs(
      p_var, p_ignore_value_in_docs);
...
Variant _GLOBAL_DEF(const PropertyInfo &p_info, const Variant &p_default,
  ...
  Variant ret = _GLOBAL_DEF(p_info.name, p_default, p_restart_if_changed,
                            p_ignore_value_in_docs, p_basic, p_internal);
  ProjectSettings::get_singleton()->set_custom_property_info(p_info);
  return ret;
}

void ProjectSettings::_add_property_info_bind(const Dictionary &p_info) {
  ERR_FAIL_COND_MSG(!p_info.has("name"),
                    "Property info is missing \"name\" field.");
  ERR_FAIL_COND_MSG(!p_info.has("type"),
                    "Property info is missing \"type\" field.");
...
const HashMap<StringName, PropertyInfo> &
ProjectSettings::get_custom_property_info() const {
...
bool ProjectSettings::_property_can_revert(const StringName &p_name) const {
  return props.has(p_name) &&
         !String(p_name).begins_with(EDITOR_SETTING_OVERRIDE_PREFIX);
}

bool ProjectSettings::_property_get_revert(const StringName &p_name,
                                           Variant &r_property) const {
  const RBMap<StringName, ProjectSettings::VariantContainer>::Element *value =
      props.find(p_name);
  if (value) {
...
void ProjectSettings::set_setting(const String &p_setting,
                                  const Variant &p_value) {
  set(p_setting, p_value);
}

Variant ProjectSettings::get_setting(const String &p_setting,
                                     const Variant &p_default_value) const {
...
bool ProjectSettings::check_changed_settings_in_group(
    const String &p_setting_prefix) const {
  for (const StringName &setting : changed_settings) {
...
void ProjectSettings::refresh_global_class_list() {
  ...
  is_global_class_list_loaded =
      false; // Make sure we read from the freshly mounted PCK.
  Array script_classes = get_global_class_list();
  for (int i = 0; i < script_classes.size(); i++) {
    Dictionary c = script_classes[i];
    if (!c.has("class") || !c.has("language") || !c.has("path") ||
        !c.has("base") || !c.has("is_abstract") || !c.has("is_tool")) {
      continue;
    }
    ScriptServer::add_global_class(c["class"], c["base"], c["language"],
                                   c["path"], c["is_abstract"], c["is_tool"]);
  }
}
...
TypedArray<Dictionary> ProjectSettings::get_global_class_list() {
  ...
  // File read succeeded or failed. If it failed, assume everything is still
  // okay. We will later receive updated class data in
  // store_global_class_list().
...
const HashMap<StringName, ProjectSettings::AutoloadInfo> &
ProjectSettings::get_autoload_list() const {
  return autoloads;
}

void ProjectSettings::add_autoload(const AutoloadInfo &p_autoload,
                                   bool p_front_insert) {
  ERR_FAIL_COND_MSG(p_autoload.name == StringName(),
                    "Trying to add autoload with no name.");
  if (p_front_insert) {
...
void ProjectSettings::remove_autoload(const StringName &p_autoload) {
  ERR_FAIL_COND_MSG(!autoloads.has(p_autoload),
                    "Trying to remove non-existent autoload.");
...
ProjectSettings::get_autoload(const StringName &p_name) const {
  ERR_FAIL_COND_V_MSG(!autoloads.has(p_name), AutoloadInfo(),
                      "Trying to get non-existent autoload.");
...
const HashMap<StringName, String> &
ProjectSettings::get_global_groups_list() const {
  return global_groups;
}

void ProjectSettings::add_global_group(const StringName &p_name,
                                       const String &p_description) {
  ERR_FAIL_COND_MSG(p_name == StringName(),
                    "Trying to add global group with no name.");
  global_groups[p_name] = p_description;
}

void ProjectSettings::remove_global_group(const StringName &p_name) {
  ERR_FAIL_COND_MSG(!global_groups.has(p_name),
                    "Trying to remove non-existent global group.");
...
void ProjectSettings::save_scene_groups_cache() {
  Ref<ConfigFile> cf;
  cf.instantiate();
  for (const KeyValue<StringName, HashSet<StringName>> &E :
       scene_groups_cache) {
    if (E.value.is_empty()) {
      continue;
    }
...
const HashMap<StringName, HashSet<StringName>> &
ProjectSettings::get_scene_groups_cache() const {
  return scene_groups_cache;
}

#ifdef TOOLS_ENABLED
void ProjectSettings::get_argument_options(const StringName &p_function,
                                           int p_idx,
                                           List<String> *r_options) const {
  const String pf = p_function;
  if (p_idx == 0) {
    if (pf == "has_setting" || pf == "set_setting" || pf == "get_setting" ||
        pf == "get_setting_with_override" || pf == "set_order" ||
        pf == "get_order" || pf == "set_initial_value" ||
        pf == "set_as_basic" || pf == "set_as_internal" ||
        pf == "set_restart_if_changed" || pf == "clear") {
      for (const KeyValue<StringName, VariantContainer> &E : props) {
...
void ProjectSettings::set_editor_setting_override(const String &p_setting,
                                                  const Variant &p_value) {
  set_setting(EDITOR_SETTING_OVERRIDE_PREFIX + p_setting, p_value);
}

bool ProjectSettings::has_editor_setting_override(
    const String &p_setting) const {
  return has_setting(EDITOR_SETTING_OVERRIDE_PREFIX + p_setting);
}

Variant
ProjectSettings::get_editor_setting_override(const String &p_setting) const {
  return get_setting(EDITOR_SETTING_OVERRIDE_PREFIX + p_setting);
}

void ProjectSettings::_bind_methods() {
  ClassDB::bind_method(D_METHOD("has_setting", "name"),
                       &ProjectSettings::has_setting);
  ClassDB::bind_method(D_METHOD("set_setting", "name", "value"),
                       &ProjectSettings::set_setting);
  ClassDB::bind_method(D_METHOD("get_setting", "name", "default_value"),
                       &ProjectSettings::get_setting, DEFVAL(Variant()));
  ClassDB::bind_method(D_METHOD("get_setting_with_override", "name"),
                       &ProjectSettings::get_setting_with_override);
  ClassDB::bind_method(D_METHOD("get_global_class_list"),
                       &ProjectSettings::get_global_class_list);
  ClassDB::bind_method(
      D_METHOD("get_setting_with_override_and_custom_features", "name",
               "features"),
      &ProjectSettings::get_setting_with_override_and_custom_features);
  ClassDB::bind_method(D_METHOD("set_order", "name", "position"),
                       &ProjectSettings::set_order);
  ClassDB::bind_method(D_METHOD("get_order", "name"),
                       &ProjectSettings::get_order);
  ClassDB::bind_method(D_METHOD("set_initial_value", "name", "value"),
                       &ProjectSettings::set_initial_value);
  ClassDB::bind_method(D_METHOD("set_as_basic", "name", "basic"),
                       &ProjectSettings::set_as_basic);
  ClassDB::bind_method(D_METHOD("set_as_internal", "name", "internal"),
                       &ProjectSettings::set_as_internal);
  ClassDB::bind_method(D_METHOD("add_property_info", "hint"),
                       &ProjectSettings::_add_property_info_bind);
  ClassDB::bind_method(D_METHOD("set_restart_if_changed", "name", "restart"),
                       &ProjectSettings::set_restart_if_changed);
  ClassDB::bind_method(D_METHOD("clear", "name"), &ProjectSettings::clear);
  ClassDB::bind_method(D_METHOD("localize_path", "path"),
                       &ProjectSettings::localize_path);
  ClassDB::bind_method(D_METHOD("globalize_path", "path"),
                       &ProjectSettings::globalize_path);
  ClassDB::bind_method(D_METHOD("save"), &ProjectSettings::save);
  ClassDB::bind_method(
      D_METHOD("load_resource_pack", "pack", "replace_files", "offset"),
      &ProjectSettings::load_resource_pack, DEFVAL(true), DEFVAL(0));

  ClassDB::bind_method(D_METHOD("save_custom", "file"),
                       &ProjectSettings::_save_custom_bnd);

  // Change tracking methods
  ClassDB::bind_method(D_METHOD("get_changed_settings"),
                       &ProjectSettings::get_changed_settings);
  ClassDB::bind_method(
      D_METHOD("check_changed_settings_in_group", "setting_prefix"),
...
void ProjectSettings::_add_builtin_input_map() {
  if (InputMap::get_singleton()) {
    HashMap<String, List<Ref<InputEvent>>> builtins(
        InputMap::get_singleton()->get_builtins());
...
ProjectSettings::ProjectSettings() {
  ...
  CRASH_COND_MSG(
      singleton != nullptr,
      "Instantiating a new ProjectSettings singleton is not supported.");
  singleton = this;

#ifdef TOOLS_ENABLED
  // Available only at runtime in editor builds. Needs to be processed before
  // anything else to work properly.
  if (!Engine::get_singleton()->is_editor_hint()) {
    String editor_features =
        OS::get_singleton()->get_environment("GODOT_EDITOR_CUSTOM_FEATURES");
  ...
  GLOBAL_DEF(PropertyInfo(Variant::DICTIONARY,
             ...
             Dictionary());
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::STRING,
                                "application/config/description",
                                PROPERTY_HINT_MULTILINE_TEXT),
                   "");
  GLOBAL_DEF_BASIC("application/config/version", "");
  GLOBAL_DEF_INTERNAL(PropertyInfo(Variant::STRING, "application/config/tags"),
                      PackedStringArray());
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::STRING, "application/run/main_scene",
  ...
  GLOBAL_DEF_BASIC(
      PropertyInfo(
          Variant::INT, "accessibility/general/accessibility_support",
          PROPERTY_HINT_ENUM,
          "Auto (When Screen Reader is Running),Always Active,Disabled"),
      0);
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::INT,
  ...
  // - Have both dimensions divisible by 8 to better play along with video
  // recording,
  // - Be displayable correctly in windowed mode on a 1366×768 display (tested
  // on Windows 10 with default settings).
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::INT,
                                "display/window/size/viewport_width",
                                PROPERTY_HINT_RANGE, "1,7680,1,or_greater"),
                   1152); // 8K resolution
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::INT,
                                "display/window/size/viewport_height",
                                PROPERTY_HINT_RANGE, "1,4320,1,or_greater"),
                   648); // 8K resolution

  GLOBAL_DEF_BASIC(
      PropertyInfo(
          Variant::INT, "display/window/size/mode", PROPERTY_HINT_ENUM,
          "Windowed,Minimized,Maximized,Fullscreen,Exclusive Fullscreen"),
      0);

  // Keep the enum values in sync with the `Window::WINDOW_INITIAL_POSITION_`
  // enum.
  GLOBAL_DEF_BASIC(
      PropertyInfo(Variant::INT, "display/window/size/initial_position_type",
                   PROPERTY_HINT_ENUM,
                   "Absolute:0,Center of Primary Screen:1,Center of Other "
                   "Screen:3,Center of Screen With Mouse Pointer:4,Center of "
                   "Screen With Keyboard Focus:5"),
      1);
  GLOBAL_DEF_BASIC(
      PropertyInfo(Variant::VECTOR2I, "display/window/size/initial_position"),
      Vector2i());
  // Keep the enum values in sync with the `DisplayServer::SCREEN_` enum.
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::INT,
                                "display/window/size/initial_screen",
                                PROPERTY_HINT_RANGE, "0,64,1,or_greater"),
                   0);
  ...
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          ...
                          PROPERTY_HINT_RANGE, "0,7680,1,or_greater"),
             0); // 8K resolution
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          "display/window/size/window_height_override",
                          PROPERTY_HINT_RANGE, "0,4320,1,or_greater"),
             0); // 8K resolution

  GLOBAL_DEF("display/window/energy_saving/keep_screen_on", true);
  GLOBAL_DEF("animation/warnings/check_invalid_track_paths", true);
  GLOBAL_DEF("animation/warnings/check_angle_interpolation_type_conflicting",
             true);
#ifndef DISABLE_DEPRECATED
  GLOBAL_DEF_RST(
  ...
  GLOBAL_DEF(PropertyInfo(Variant::INT, "audio/general/default_playback_type",
                          PROPERTY_HINT_ENUM, "Stream,Sample"),
             0);
  GLOBAL_DEF(PropertyInfo(Variant::INT,
  ...
  GLOBAL_DEF(
      PropertyInfo(
          Variant::INT, "audio/general/ios/session_category",
          PROPERTY_HINT_ENUM,
          "Ambient,Multi Route,Play and Record,Playback,Record,Solo Ambient"),
      0);
  GLOBAL_DEF("audio/general/ios/mix_with_others", false);

  _add_builtin_input_map();

  // Keep the enum values in sync with the `DisplayServer::ScreenOrientation`
  // enum.
  custom_prop_info["display/window/handheld/orientation"] = PropertyInfo(
      Variant::INT, "display/window/handheld/orientation", PROPERTY_HINT_ENUM,
  ...
  custom_prop_info["display/window/vsync/vsync_mode"] =
      PropertyInfo(Variant::INT, "display/window/vsync/vsync_mode",
                   PROPERTY_HINT_ENUM, "Disabled,Enabled,Adaptive,Mailbox");

  GLOBAL_DEF("display/window/frame_pacing/android/enable_frame_pacing", true);
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          "display/window/frame_pacing/android/swappy_mode",
                          PROPERTY_HINT_ENUM,
                          "pipeline_forced_on,auto_fps_pipeline_forced_on,auto_"
                          "fps_auto_pipeline"),
             2);

#ifdef DISABLE_DEPRECATED
  custom_prop_info["rendering/driver/threads/thread_model"] =
      PropertyInfo(Variant::INT, "rendering/driver/threads/thread_model",
                   PROPERTY_HINT_ENUM, "Safe:1,Separate");
#else
  custom_prop_info["rendering/driver/threads/thread_model"] =
      PropertyInfo(Variant::INT, "rendering/driver/threads/thread_model",
                   PROPERTY_HINT_ENUM, "Unsafe (deprecated),Safe,Separate");
#endif
  ...
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::STRING, "display/window/stretch/mode",
                                PROPERTY_HINT_ENUM,
  ...
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::STRING,
                   ...
                   "keep");
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::FLOAT, "display/window/stretch/scale",
                                PROPERTY_HINT_RANGE, "0.5,8.0,0.01"),
                   1.0);
  GLOBAL_DEF_BASIC(PropertyInfo(Variant::STRING,
                                "display/window/stretch/scale_mode",
                                PROPERTY_HINT_ENUM, "fractional,integer"),
                   "fractional");

  GLOBAL_DEF(PropertyInfo(Variant::INT, "debug/settings/profiler/max_functions",
                          PROPERTY_HINT_RANGE, "128,65535,1"),
             16384);
  GLOBAL_DEF_RST(
      PropertyInfo(Variant::INT,
                   "debug/settings/profiler/max_timestamp_query_elements",
                   PROPERTY_HINT_RANGE, "256,65535,1"),
      256);

  GLOBAL_DEF(PropertyInfo(Variant::BOOL,
  ...
  GLOBAL_DEF("debug/settings/crash_handler/message",
             String("Please include this when reporting the bug to the project "
                    "developer."));
  ...
  GLOBAL_DEF_BASIC(
      PropertyInfo(Variant::INT,
                   ...
                   "Locale,Left-to-Right,Right-to-Left,Based on System Locale"),
      0);
  GLOBAL_DEF_BASIC("internationalization/rendering/root_node_auto_translate",
                   true);

  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          "gui/timers/incremental_search_max_interval_msec",
                          PROPERTY_HINT_RANGE, "0,10000,1,or_greater"),
             2000);
  GLOBAL_DEF(PropertyInfo(Variant::FLOAT, "gui/timers/tooltip_delay_sec",
                          PROPERTY_HINT_RANGE, "0,5,0.01,or_greater"),
             0.5);
#ifdef TOOLS_ENABLED
  ...
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          ...
                          PROPERTY_HINT_ENUM,
                          "Never,Control Supports Keyboard Input,Always"),
             1);
  ...
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          ...
                          "texture_upload_region_size_px",
                          PROPERTY_HINT_RANGE, "1,256,1,or_greater"),
             64);
  GLOBAL_DEF(PropertyInfo(Variant::INT,
                          "rendering/rendering_device/staging_buffer/"
                          "texture_download_region_size_px",
                          PROPERTY_HINT_RANGE, "1,256,1,or_greater"),
             64);
  GLOBAL_DEF_RST(
      ...
      true);
  GLOBAL_DEF(PropertyInfo(
                 Variant::FLOAT,
                 "rendering/rendering_device/pipeline_cache/save_chunk_size_mb",
                 PROPERTY_HINT_RANGE, "0.000001,64.0,0.001,or_greater"),
             3.0);
  GLOBAL_DEF(
      PropertyInfo(Variant::INT,
  ...
  GLOBAL_DEF_RST("rendering/rendering_device/d3d12/max_resource_descriptors",
                 65536);
  custom_prop_info
      ["rendering/rendering_device/d3d12/max_resource_descriptors"] =
          PropertyInfo(
              Variant::INT,
              "rendering/rendering_device/d3d12/max_resource_descriptors",
              PROPERTY_HINT_RANGE, "512,1000000");
  GLOBAL_DEF_RST("rendering/rendering_device/d3d12/max_sampler_descriptors",
                 1024);
  custom_prop_info["rendering/rendering_device/d3d12/max_sampler_descriptors"] =
      PropertyInfo(Variant::INT,
                   "rendering/rendering_device/d3d12/max_sampler_descriptors",
                   PROPERTY_HINT_RANGE, "256,2048");
  ...
  GLOBAL_DEF_RST(
      PropertyInfo(Variant::INT,
                   ...
                   PROPERTY_HINT_RANGE, "0,10000,1,or_greater,hide_control"),
      618);
  ...
  GLOBAL_DEF_BASIC(
      PropertyInfo(Variant::INT,
                   ...
                   PROPERTY_HINT_ENUM, "Disable,Enable,Mirror"),
      0);
  ...
  GLOBAL_DEF_BASIC("input_devices/pointing/android/disable_scroll_deadzone",
                   false);

  // These properties will not show up in the dialog. If you want to exclude
  // whole groups, use add_hidden_prefix().
  GLOBAL_DEF_INTERNAL("application/config/features", PackedStringArray());
  GLOBAL_DEF_INTERNAL("internationalization/locale/translation_remaps",
                      PackedStringArray());
  GLOBAL_DEF_INTERNAL("internationalization/locale/translations",
                      PackedStringArray());
  GLOBAL_DEF_INTERNAL("internationalization/locale/translations_pot_files",
                      PackedStringArray());
  GLOBAL_DEF_INTERNAL(
      "internationalization/locale/translation_add_builtin_strings_to_pot",
      false);
  ...
  GLOBAL_DEF("navigation/avoidance/thread_model/avoidance_use_multiple_threads",
             true);
  GLOBAL_DEF(
  ...
  GLOBAL_DEF("navigation/baking/thread_model/baking_use_multiple_threads",
             true);
  GLOBAL_DEF("navigation/baking/thread_model/baking_use_high_priority_threads",
             true);
