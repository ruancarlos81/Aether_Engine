...
void Engine::_update_time_scale() {
  ...
  max_user_physics_steps_per_frame =
      MAX(max_physics_steps_per_frame,
          max_physics_steps_per_frame * _user_time_scale);
}

void Engine::set_physics_ticks_per_second(int p_ips) {
  ERR_FAIL_COND_MSG(p_ips <= 0,
...
void Engine::set_max_physics_steps_per_frame(int p_max_physics_steps) {
  ERR_FAIL_COND_MSG(
      p_max_physics_steps <= 0,
      "Maximum number of physics steps per frame must be greater than 0.");
...
uint64_t Engine::get_frames_drawn() { return frames_drawn; }
...
static Array array_from_info_count(const char *const *info_list,
                                   int info_count) {
  Array arr;
  ...
  for (int component_index = 0; component_index < COPYRIGHT_INFO_COUNT;
    ...
    for (int i = 0; i < cp_info.part_count; i++) {
      ...
      part_dict["copyright"] = array_from_info_count(
          cp_part.copyright_statements, cp_part.copyright_count);
...
void Engine::add_singleton(const Singleton &p_singleton) {
  ERR_FAIL_COND_MSG(
      singleton_ptrs.has(p_singleton.name),
      vformat("Can't register singleton '%s' because it already exists.",
              p_singleton.name));
...
Object *Engine::get_singleton_object(const StringName &p_name) const {
  ...
  ERR_FAIL_COND_V_MSG(
      !E, nullptr,
      vformat("Failed to retrieve non-existent singleton '%s'.", p_name));

#ifdef TOOLS_ENABLED
  if (!is_editor_hint() && is_singleton_editor_only(p_name)) {
    ERR_FAIL_V_MSG(
        nullptr,
        vformat("Can't retrieve singleton '%s' outside of editor.", p_name));
  }
#endif
...
Engine::Singleton::Singleton(const StringName &p_name, Object *p_ptr,
  ...
  if (rc && !rc->is_referenced()) {
    WARN_PRINT("You must use Ref<> to ensure the lifetime of a RefCounted "
               "object intended to be used as a singleton.");
  }
#endif
}
