...
class ProjectSettings : public Object {
  ...
  // Starting version from 1 ensures that all callers can reset their tested
  // version to 0, and will always detect the initial project settings as a
  // "change".
  uint32_t _version = 1;
...
public:
  ...
  // This constant is used to make the ".godot" folder and paths like
  // "res://.godot/editor".
  ...
  const static PackedStringArray
  get_unsupported_features(const PackedStringArray &p_project_features);
#endif // TOOLS_ENABLED
...
protected:
  ...
  HashMap<StringName, LocalVector<Pair<StringName, StringName>>>
      feature_overrides;
  ...
  Error _load_settings_text_or_binary(const String &p_text_path,
                                      const String &p_bin_path);

  Error _save_settings_text(const String &p_file,
                            const RBMap<String, List<String>> &props,
  ...
  const static PackedStringArray
  _trim_to_supported_features(const PackedStringArray &p_project_features);
#endif // TOOLS_ENABLED

  void _convert_to_last_version(int p_from_version);

  bool load_resource_pack(const String &p_pack, bool p_replace_files,
                          int p_offset);
  bool _load_resource_pack(const String &p_pack, bool p_replace_files = true,
                           int p_offset = 0, bool p_main_pack = false);

  void _add_property_info_bind(const Dictionary &p_info);

  Error _setup(const String &p_path, const String &p_main_pack,
               bool p_upwards = false, bool p_ignore_override = false);
...
public:
  ...
  Variant get_setting(const String &p_setting,
                      const Variant &p_default_value = Variant()) const;
  ...
  get_scene_groups_cache() const;
  void add_scene_groups_cache(const StringName &p_path,
                              const HashSet<StringName> &p_cache);
...
#ifdef TOOLS_ENABLED
  virtual void get_argument_options(const StringName &p_function, int p_idx,
                                    List<String> *r_options) const override;
#endif

  void set_editor_setting_override(const String &p_setting,
...
Variant _GLOBAL_DEF(const String &p_var, const Variant &p_default,
                    ...
                    bool p_ignore_value_in_docs = false, bool p_basic = false,
...
#define GLOBAL_GET(m_var)                                                      \
  ProjectSettings::get_singleton()->get_setting_with_override(m_var)

#define GLOBAL_DEF_BASIC(m_var, m_value)                                       \
  _GLOBAL_DEF(m_var, m_value, false, false, true)
#define GLOBAL_DEF_RST_BASIC(m_var, m_value)                                   \
  _GLOBAL_DEF(m_var, m_value, true, false, true)
#define GLOBAL_DEF_NOVAL_BASIC(m_var, m_value)                                 \
  _GLOBAL_DEF(m_var, m_value, false, true, true)
#define GLOBAL_DEF_RST_NOVAL_BASIC(m_var, m_value)                             \
  _GLOBAL_DEF(m_var, m_value, true, true, true)

#define GLOBAL_DEF_INTERNAL(m_var, m_value)                                    \
  _GLOBAL_DEF(m_var, m_value, false, false, false, true)
#define GLOBAL_GET_CACHED(m_type, m_setting_name)                              \
  ([](const char *p_name) -> m_type {                                          \
    static_assert(std::is_trivially_destructible<m_type>::value,               \
                  "GLOBAL_GET_CACHED must use a trivial type that allows "     \
                  "static lifetime.");                                         \
    static m_type _ggc_local_var;                                              \
    static uint32_t _ggc_local_version = 0;                                    \
    static SpinLock _ggc_spin;                                                 \
    uint32_t _ggc_new_version =                                                \
        ProjectSettings::get_singleton()->get_version();                       \
    if (_ggc_local_version != _ggc_new_version) {                              \
      _ggc_spin.lock();                                                        \
      _ggc_local_version = _ggc_new_version;                                   \
      _ggc_local_var =                                                         \
          ProjectSettings::get_singleton()->get_setting_with_override(p_name); \
