...
public:
  static Ref<TLSOptions>
  client(Ref<X509Certificate> p_trusted_chain = Ref<X509Certificate>(),
         const String &p_common_name_override = String());
  static Ref<TLSOptions> client_unsafe(Ref<X509Certificate> p_trusted_chain);
  static Ref<TLSOptions> server(Ref<CryptoKey> p_own_key,
...
public:
  ...
  virtual Error start(HashingContext::HashType p_hash_type,
                      const PackedByteArray &p_key) = 0;
...
public:
  ...
  virtual Ref<X509Certificate> generate_self_signed_certificate(
      Ref<CryptoKey> p_key, const String &p_issuer_name,
      const String &p_not_before, const String &p_not_after) = 0;

  virtual Vector<uint8_t> sign(HashingContext::HashType p_hash_type,
                               const Vector<uint8_t> &p_hash,
                               Ref<CryptoKey> p_key) = 0;
  virtual bool verify(HashingContext::HashType p_hash_type,
                      const Vector<uint8_t> &p_hash,
                      const Vector<uint8_t> &p_signature,
                      Ref<CryptoKey> p_key) = 0;
  virtual Vector<uint8_t> encrypt(Ref<CryptoKey> p_key,
                                  const Vector<uint8_t> &p_plaintext) = 0;
  virtual Vector<uint8_t> decrypt(Ref<CryptoKey> p_key,
                                  const Vector<uint8_t> &p_ciphertext) = 0;

  PackedByteArray hmac_digest(HashingContext::HashType p_hash_type,
                              const PackedByteArray &p_key,
                              const PackedByteArray &p_msg);

  // Compares two PackedByteArrays for equality without leaking timing
  ...
  bool constant_time_compare(const PackedByteArray &p_trusted,
...
public:
  ...
  load(const String &p_path, const String &p_original_path = "",
  ...
  // Treat certificates as text files, do not generate a `*.{crt,key,pub}.uid`
  // file.
  virtual ResourceUID::ID
  get_resource_uid(const String &p_path) const override {
...
public:
  ...
  virtual void
  get_recognized_extensions(const Ref<Resource> &p_resource,
