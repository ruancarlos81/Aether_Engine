...
Error AESContext::start(Mode p_mode, const PackedByteArray &p_key,
                        const PackedByteArray &p_iv) {
  ERR_FAIL_COND_V_MSG(
      mode != MODE_MAX, ERR_ALREADY_IN_USE,
      "AESContext already started. Call 'finish' before starting a new one.");
  ERR_FAIL_COND_V_MSG(p_mode < 0 || p_mode >= MODE_MAX, ERR_INVALID_PARAMETER,
  ...
  if (p_mode == MODE_CBC_ENCRYPT || p_mode == MODE_CBC_DECRYPT) {
    ERR_FAIL_COND_V_MSG(
        p_iv.size() != 16, ERR_INVALID_PARAMETER,
...
PackedByteArray AESContext::update(const PackedByteArray &p_src) {
  ERR_FAIL_COND_V_MSG(
      mode < 0 || mode >= MODE_MAX, PackedByteArray(),
...
PackedByteArray AESContext::get_iv_state() {
  ERR_FAIL_COND_V_MSG(mode != MODE_CBC_ENCRYPT && mode != MODE_CBC_DECRYPT,
                      PackedByteArray(),
                      "Calling 'get_iv_state' only makes sense when the "
                      "context is started in CBC mode.");
  PackedByteArray out;
  out.append_array(iv);
  return out;
}
...
void AESContext::_bind_methods() {
  ClassDB::bind_method(D_METHOD("start", "mode", "key", "iv"),
                       &AESContext::start, DEFVAL(PackedByteArray()));
